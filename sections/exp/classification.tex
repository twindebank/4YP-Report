...
\label{sec:exp-clf}
    ...
    \label{subsec:exp-clf-known}  
        ...
        \label{subsubsec:exp-clf-known-featnorm}
            Feature normalisation, or \textit{whitening}, is done with reuse of constants for reasons discussed in section \ref{subsec:pl-data-software}. The impact of normalisation depends on the nature of the classifier being used and how similarity/ or distance is defined. For a random forest classifier, decisions whether to split a node are based on the information/entropy of the feature (discussed further in section \ref{subsec:pl-clf-sup}), a criteria that is invariant to monotonic transforms such as feature scaling \cite{Hastie2009}, so normalisation will not be applied for the random forest.
            ...
            These conclusions have been confirmed in brief trials where both the random forest and naive Bayes classifiers were unaffected by feature normalisation, and the SVM without normalisation was $\sim$10 times slower to train with resultant $F_{1}$ scores reduced by at least 0.2.
    ...
    \label{subsec:exp-clf-opt}
        ...
        \label{subsubsec:exp-clf-opt-featsel}
            ...
            \begin{align}
                \text{\code{winstep}} < \text{\code{winlen, winstep}} > \frac{1}{\text{fs}} \text{, and \code{winlen}} < \text{\code{siglen}}.
            \end{align}
