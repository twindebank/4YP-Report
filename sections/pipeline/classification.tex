...
\label{sec:pl-clf}
    ...
    \label{subsec:pl-clf-sup}
        ...
        The last classifier, a support vector machine (SVM) \cite{Vapnik1998}, is also a discriminative classifier. If each row of the feature matrix $X\in\mathbb{R}^{n\times d}$ is treated as a coordinate, then the data can be visualised in the feature-space over $d$ dimensions. SVMs operate to find a hyperplane separating the points in the feature-space that best divides the dataset into two classes. The simplified case of two dimensions is shown in figure X, where a hyperplane 
        \begin{talign}
            \vec{w}\cdot\vec{x}-b=0    
        \end{talign}
        divides the two dimensional feature-space. The points closest to the hyperplane determine its direction and intercept, these are called the support vectors; the distance between the support vectors and the hyperplane is defined as the margin, bounded by the hyperplanes
        \begin{talign}
        ...
        \begin{talign}
            y_k(\vec{w_k}\vec{x_k}-b) \geq 1 for 1\leq k\leq n.
